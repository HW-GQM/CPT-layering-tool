...
tab1, tab2 = st.tabs(["Editor", "3D Model"])
...
with tab1:
    ...
    with st.sidebar.expander("Options"):
        ...
        "Select exactly 4 parameters", 
        options=["Qt", "Fr", "Bq", "Ic"], 
        default=[], 
        max_selections=2  # Optionally limit to 4 selections
)
...
with tab2:
    @st.cache_data
    def load_plot_df():
        plot_df = pd.read_excel("training data/EnBW cpt parameters_3d plot.xlsx")
        return plot_df

    @st.cache_data
    def threeD_plot(plot_df):
        trace = go.Scatter3d(
        x=plot_df['East'],
        y=plot_df['North'],
        z=plot_df['Depth (m)'],
        mode='markers',
        marker=dict(
            size=plot_df["Fr scaled"]*20+7,  # Marker size scaled by F_r
            color=plot_df['Qt'],  # Color by Q_t
            colorscale='Viridis',  # Color scale for Q_t
            colorbar=dict(title='Qt'),
            opacity=0.8,
            cmin=0,
            cmax=50,
            symbol="circle",
        ...
        layout = go.Layout(
            ...
            margin=dict(
                ...
                title='3D Spatial Visualization of CPT Data'
        )
